...
public class PidgeyBird: NSObject {
    ...
    private var requests: [PidgeyRequest] = []
...
extension PidgeyBird {
    public func requestWithID(requestID: Int) -> PidgeyRequest?{
        let filteredRequests = requests.filter {$0.taskIdentifier == requestID}
        return filteredRequests.first
    }
    
    public func cancelAllRequests(){
        requests.forEach { (request:PidgeyRequest) in
            request.cancel()
        }
    }
}
